...
export default function SandControls({
  ...
  return (
    <div className="flex flex-col gap-3 bg-gray-900/50 p-4 rounded border border-gray-800/60 text-xs w-full md:w-44 select-none self-stretch md:self-start shrink-0">
      ...
      {elements.map(([name, id, style]) => (
        <div key={id} className="flex flex-col gap-2">
          <button
            ...
            className={`w-full px-4 py-3 border rounded tracking-widest transition-all active:scale-95 cursor-pointer font-bold text-left ${style} ${
              activeElement === id
          ...
          {/* Slider renders when Eraser is active */}
          {id === 0 && activeElement === 0 && (
            <div className="mt-1 p-2 bg-gray-950/60 rounded border border-rose-950/40 flex flex-col gap-1.5 transition-all">
      ...
      <div className="mt-4 border-t border-gray-800 pt-3">
        <button
          onClick={onClear}
          className="w-full px-4 py-3 border rounded tracking-widest transition-all active:scale-95 cursor-pointer font-bold text-gray-400 border-gray-500/20 bg-gray-500/5 hover:bg-rose-500/10 hover:text-rose-400 hover:border-rose-500/30"
        >
          RESET
        </button>
      </div>
